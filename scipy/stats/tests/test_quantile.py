...
def quantile_reference_last_axis(x, p, nan_policy, method):
    ...
    if method == 'harrell-davis':
        ...
        preset_options = dict(
            winsor=dict(round_left=np.round, round_right=np.round),
            winsor_less=dict(round_left=np.floor, round_right=np.ceil),
            winsor_more=dict(round_left=np.ceil, round_right=np.floor),
        )
        options = preset_options.get(method)
        round_left = options.get('round_left', np.round)
        round_right = options.get('round_right', np.round)
        res = winsor_reference_1d(np.sort(x), p, round_left, round_right)
    else:
...
def winsor_reference_1d(y, p, round_left, round_right):
    # Adapted directly from the documentation
    # Note: `y` is the sorted data array
    n = len(y)
    j = int(round_left(p * n) if p < 0.5 else round_right(n * p - 1))
    return y[j]
...
class TestQuantile:
    ...
    @pytest.mark.parametrize('method',
         ['inverted_cdf', 'averaged_inverted_cdf', 'closest_observation',
          ...
          'harrell-davis', 'winsor', 'winsor_less', 'winsor_more'])
    ...
    def test_against_reference(self, method, shape_x, shape_p, axis, xp):
        ...
        preset_options = dict(
            winsor=None,
            winsor_less=dict(round_left=xp.floor, round_right=xp.ceil),
            winsor_more=dict(round_left=xp.ceil, round_right=xp.floor),
        )
        options = preset_options.get(method, None)
        method = 'winsor' if method.startswith('winsor_') else method
        x, p = xp.asarray(x, dtype=dtype), xp.asarray(p, dtype=dtype)
        res = stats.quantile(x, p, method=method, axis=axis, options=options)
    ...
    @pytest.mark.parametrize('method', ['linear', 'harrell-davis', 'winsor'])
