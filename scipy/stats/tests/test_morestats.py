...
class TransformTest:
    def test_nan_policy(self):
        ...
        np.testing.assert_equal(res, special_transform(x, 1.))

        res = transform(x, lmb, nan_policy='omit')
        np.testing.assert_equal(res, transform(x, lmb, nan_policy='propagate'))
        ...
        ref = transform(x, lmb)
        np.testing.assert_equal(res[0], ref)
        np.testing.assert_equal(res[1], lmb)
...
class TestYeojohnsonNormmax:
    ...
    def test_nan_policy(self):
        ...
        with pytest.raises(ValueError, match="The input contains nan values"):
            stats.boxcox_normmax(x, nan_policy='raise')

        res = stats.boxcox_normmax(x, nan_policy='propagate')
        np.testing.assert_equal(res, np.nan)

        res = stats.boxcox_normmax(x, nan_policy='omit')
        ref = stats.boxcox_normmax(x[~np.isnan(x)])
        np.testing.assert_equal(res, ref)
