...
class TestWilcoxon:
    ...
    def test_accuracy_wilcoxon(self):
        ...
        T, p = stats.wilcoxon(x, y, "pratt", mode="asymptotic", correction=False)
        assert_allclose(T, 423)
        assert_allclose(p, 0.0031724568006762576)

        T, p = stats.wilcoxon(x, y, "zsplit", mode="asymptotic", correction=False)
        assert_allclose(T, 441)
        assert_allclose(p, 0.0032145343172473055)

        T, p = stats.wilcoxon(x, y, "wilcox", mode="asymptotic", correction=False)
        ...
        # Test that `mode='approx'` still does the same thing as 'asymptotic'
        T2, p2 = stats.wilcoxon(x, y, correction=True, mode="approx")
        assert_equal(T2, T)
        assert_equal(p2, p)
    ...
    def test_wilcoxon_tie(self):
        ...
        #   > result$p.value
        #   [1] 0.001904195
        stat, p = stats.wilcoxon([0.1] * 10, mode="asymptotic", correction=False)
        expected_p = 0.001565402
        assert_equal(stat, 0)
        assert_allclose(p, expected_p, rtol=1e-6)

        stat, p = stats.wilcoxon([0.1] * 10, correction=True, mode="asymptotic")
    ...
    def test_auto(self):
        ...
        # use asymptotic approximation for samples > 25
        d = np.arange(1, 52)
