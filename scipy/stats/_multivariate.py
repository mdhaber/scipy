...
class normal_inverse_gaussian_gen(multi_rv_generic):
    def rvs(self, mu=0, lmbda=1, a=1, b=1, size=None, random_state=None):
        random_state = self._get_random_state(random_state)
        s2 = stats.invgamma(a, scale=b).rvs(size=size, random_state=random_state)
        x = stats.norm(loc=mu, scale=(s2 / lmbda)**0.5).rvs(size=size, random_state=random_state)
        return x, s2
    ...
    def __call__(self, mu=0, lmbda=1, a=1, b=1, seed=None):
        return normal_inverse_gaussian_frozen(mu, lmbda, a, b, seed=seed)

normal_inverse_gaussian = normal_inverse_gaussian_gen()

class normal_inverse_gaussian_frozen(multi_rv_frozen):

    def __init__(self, mu=0, lmbda=1, a=1, b=1, seed=None):
        self._dist = normal_inverse_gaussian_gen(seed)
